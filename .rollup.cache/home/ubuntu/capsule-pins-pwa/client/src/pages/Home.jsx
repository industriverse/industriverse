import { __assign, __awaiter, __generator, __spreadArray } from "tslib";
import { useState, useCallback, useMemo } from 'react';
import CapsulePill from '@/components/CapsulePill';
import { Button } from '@/components/ui/button';
import { toast } from 'sonner';
import { useCapsuleWebSocket } from '@/hooks/useCapsuleWebSocket';
import { getAPI } from '@/services/CapsuleAPI';
import CapsuleFilters from '@/components/CapsuleFilters';
import { Link } from 'wouter';
// Mock capsule data for demonstration
var mockCapsules = [
    {
        id: 'cap-001',
        title: 'Thermal Anomaly Detected',
        description: 'Temperature spike detected in reactor core zone A3. Immediate attention required.',
        status: 'critical',
        priority: 5,
        timestamp: new Date(Date.now() - 5 * 60000).toISOString(),
        source: 'thermal_sampler',
        metadata: {
            zone: 'A3',
            temperature: '385°C',
            threshold: '350°C',
            deviation: '+10%'
        },
        actions: ['mitigate', 'inspect', 'escalate'],
        utid: 'UTID-thermal-a3-001',
        proofId: 'PRF-thermal-001',
        energyConsumed: 245.8,
        carbonFootprint: 0.1229
    },
    {
        id: 'cap-002',
        title: 'Plasma Dynamics Optimization',
        description: 'WorldModel suggests parameter adjustment for improved efficiency.',
        status: 'warning',
        priority: 3,
        timestamp: new Date(Date.now() - 15 * 60000).toISOString(),
        source: 'world_model',
        metadata: {
            efficiency: '87.3%',
            potential: '92.1%',
            gain: '+4.8%'
        },
        actions: ['inspect', 'acknowledge', 'dismiss'],
        utid: 'UTID-plasma-opt-002',
        energyConsumed: 156.2,
        carbonFootprint: 0.0781
    },
    {
        id: 'cap-003',
        title: 'Edge Adaptation Complete',
        description: 'MicroAdapt successfully adapted to new operating regime.',
        status: 'resolved',
        priority: 2,
        timestamp: new Date(Date.now() - 30 * 60000).toISOString(),
        source: 'microadapt_edge',
        metadata: {
            regime: 'high-throughput',
            adaptation_time: '1.2s',
            accuracy: '98.7%'
        },
        actions: ['inspect', 'dismiss'],
        utid: 'UTID-adapt-003',
        energyConsumed: 89.4,
        carbonFootprint: 0.0447
    },
    {
        id: 'cap-004',
        title: 'System Health Check',
        description: 'All systems operating within normal parameters.',
        status: 'active',
        priority: 1,
        timestamp: new Date(Date.now() - 2 * 60000).toISOString(),
        source: 'system_monitor',
        metadata: {
            cpu: '45%',
            memory: '62%',
            network: 'optimal'
        },
        actions: ['inspect', 'dismiss'],
        energyConsumed: 12.3,
        carbonFootprint: 0.0062
    }
];
export default function Home() {
    var _this = this;
    var _a = useState(mockCapsules), capsules = _a[0], setCapsules = _a[1];
    var _b = useState({
        search: '',
        status: 'all',
        priority: 'all',
        sortBy: 'timestamp',
        sortOrder: 'desc'
    }), filters = _b[0], setFilters = _b[1];
    // WebSocket configuration (using mock endpoint for demo)
    var wsUrl = import.meta.env.VITE_CAPSULE_GATEWAY_WS || 'wss://capsule-gateway.industriverse.io/ws';
    var authToken = import.meta.env.VITE_AUTH_TOKEN || '';
    // Handle capsule updates from WebSocket
    var handleCapsuleUpdate = useCallback(function (update) {
        setCapsules(function (prev) { return prev.map(function (c) {
            return c.id === update.capsuleId
                ? __assign(__assign({}, c), update.updates) : c;
        }); });
        toast.info('Capsule Updated', {
            description: "Capsule ".concat(update.capsuleId, " has been updated")
        });
    }, []);
    var handleCapsuleNew = useCallback(function (capsule) {
        setCapsules(function (prev) { return __spreadArray([capsule], prev, true); });
        toast.success('New Capsule', {
            description: capsule.title
        });
    }, []);
    var handleCapsuleRemoved = useCallback(function (capsuleId) {
        setCapsules(function (prev) { return prev.filter(function (c) { return c.id !== capsuleId; }); });
        toast.info('Capsule Removed', {
            description: "Capsule ".concat(capsuleId, " removed")
        });
    }, []);
    var handleWebSocketError = function (error) {
        console.error('WebSocket error:', error);
        // Don't show toast in development mode (expected when using mock data)
        if (!import.meta.env.DEV) {
            toast.error('Connection Error', {
                description: 'Failed to connect to real-time updates'
            });
        }
    };
    // WebSocket connection (disabled for demo, will connect to real endpoint in production)
    // Disable WebSocket in development when using mock data
    var enableWebSocket = import.meta.env.VITE_ENABLE_WEBSOCKET === 'true';
    var _c = useCapsuleWebSocket({
        url: wsUrl,
        authToken: 'demo_token',
        autoConnect: enableWebSocket, // Only auto-connect if explicitly enabled
        onCapsuleUpdate: handleCapsuleUpdate,
        onCapsuleNew: handleCapsuleNew,
        onCapsuleRemoved: handleCapsuleRemoved,
        onError: handleWebSocketError
    }), connectionState = _c.connectionState, connect = _c.connect, disconnect = _c.disconnect, isConnected = _c.isConnected;
    // Filter and sort capsules
    var filteredCapsules = useMemo(function () {
        var filtered = __spreadArray([], capsules, true);
        // Apply search filter
        if (filters.search) {
            var searchLower_1 = filters.search.toLowerCase();
            filtered = filtered.filter(function (c) {
                return c.title.toLowerCase().includes(searchLower_1) ||
                    c.source.toLowerCase().includes(searchLower_1) ||
                    c.description.toLowerCase().includes(searchLower_1);
            });
        }
        // Apply status filter
        if (filters.status !== 'all') {
            filtered = filtered.filter(function (c) { return c.status === filters.status; });
        }
        // Apply priority filter
        if (filters.priority !== 'all') {
            var priorityNum_1 = parseInt(filters.priority.substring(1));
            filtered = filtered.filter(function (c) { return c.priority === priorityNum_1; });
        }
        // Apply sorting
        filtered.sort(function (a, b) {
            var comparison = 0;
            switch (filters.sortBy) {
                case 'timestamp':
                    comparison = new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime();
                    break;
                case 'priority':
                    comparison = a.priority - b.priority;
                    break;
                case 'status':
                    var statusOrder = { critical: 0, warning: 1, active: 2, resolved: 3, dismissed: 4 };
                    comparison = statusOrder[a.status] - statusOrder[b.status];
                    break;
            }
            return filters.sortOrder === 'asc' ? comparison : -comparison;
        });
        return filtered;
    }, [capsules, filters]);
    var handleAction = function (action, capsuleId) { return __awaiter(_this, void 0, void 0, function () {
        var capsule, api, response, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    capsule = capsules.find(function (c) { return c.id === capsuleId; });
                    if (!capsule)
                        return [2 /*return*/];
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    // Optimistic UI update
                    setCapsules(function (prev) { return prev.map(function (c) {
                        return c.id === capsuleId
                            ? __assign(__assign({}, c), { status: action === 'dismiss' ? 'dismissed' : c.status }) : c;
                    }); });
                    api = getAPI();
                    return [4 /*yield*/, api.executeAction({
                            capsuleId: capsuleId,
                            action: action,
                            metadata: {
                                timestamp: new Date().toISOString()
                            }
                        })];
                case 2:
                    response = _a.sent();
                    toast.success("Action: ".concat(action), {
                        description: "Executed ".concat(action, " on ").concat(capsule.title)
                    });
                    console.log('Action response:', response);
                    return [3 /*break*/, 4];
                case 3:
                    error_1 = _a.sent();
                    // Revert optimistic update on error
                    setCapsules(function (prev) { return prev.map(function (c) {
                        return c.id === capsuleId
                            ? capsule
                            : c;
                    }); });
                    toast.error('Action Failed', {
                        description: error_1 instanceof Error ? error_1.message : 'Unknown error'
                    });
                    console.error('Action error:', error_1);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    }); };
    return (<div className="min-h-screen bg-background">
      {/* Header */}
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-foreground">Capsule Pins</h1>
              <p className="text-sm text-muted-foreground">Real-time industrial intelligence</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <div className={"h-2 w-2 rounded-full ".concat(isConnected ? 'bg-emerald-400' : 'bg-slate-500', " animate-pulse")}/>
                <span className="text-xs text-muted-foreground">
                  {connectionState}
                </span>
              </div>
              <Link href="/settings">
                <Button variant="outline" size="sm">
                  Settings
                </Button>
              </Link>
              <Button variant="default" size="sm" onClick={isConnected ? disconnect : connect}>
                {isConnected ? 'Disconnect' : 'Connect'}
              </Button>
            </div>
          </div>
        </div>
      </header>
      
      {/* Main Content */}
      <main className="container py-8">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* Stats */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-card border border-border rounded-lg p-4">
              <p className="text-sm text-muted-foreground">Active Capsules</p>
              <p className="text-2xl font-bold text-foreground mt-1">
                {capsules.filter(function (c) { return c.status === 'active'; }).length}
              </p>
            </div>
            <div className="bg-card border border-border rounded-lg p-4">
              <p className="text-sm text-muted-foreground">Warnings</p>
              <p className="text-2xl font-bold text-amber-400 mt-1">
                {capsules.filter(function (c) { return c.status === 'warning'; }).length}
              </p>
            </div>
            <div className="bg-card border border-border rounded-lg p-4">
              <p className="text-sm text-muted-foreground">Critical</p>
              <p className="text-2xl font-bold text-rose-400 mt-1">
                {capsules.filter(function (c) { return c.status === 'critical'; }).length}
              </p>
            </div>
            <div className="bg-card border border-border rounded-lg p-4">
              <p className="text-sm text-muted-foreground">Resolved</p>
              <p className="text-2xl font-bold text-emerald-400 mt-1">
                {capsules.filter(function (c) { return c.status === 'resolved'; }).length}
              </p>
            </div>
          </div>
          
          {/* Filters */}
          <CapsuleFilters filters={filters} onFiltersChange={setFilters} totalCount={capsules.length} filteredCount={filteredCapsules.length}/>
          
          {/* Capsules List */}
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">Live Capsules</h2>
              <p className="text-sm text-muted-foreground">
                {filteredCapsules.length} total
              </p>
            </div>
            
            {filteredCapsules.map(function (capsule) { return (<CapsulePill key={capsule.id} capsule={capsule} onAction={handleAction}/>); })}
          </div>
          
          {/* Info Section */}
          <div className="bg-card border border-border rounded-lg p-6 mt-8">
            <h3 className="text-lg font-semibold text-foreground mb-3">About Capsule Pins</h3>
            <div className="space-y-2 text-sm text-muted-foreground">
              <p>
                Capsule Pins provide real-time visibility into your industrial intelligence ecosystem.
                Each capsule represents an actionable insight from the Industriverse platform.
              </p>
              <p>
                Powered by the complete vertical stack: Remix Lab → A2A → MCP → Thermodynamic Computing → DAC Factory
              </p>
              <p className="text-xs text-muted-foreground/70 mt-4">
                Week 7 - Progressive Web App Implementation
              </p>
            </div>
          </div>
        </div>
      </main>
    </div>);
}
//# sourceMappingURL=Home.jsx.map