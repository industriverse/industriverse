...
export function DysonLayout({ children }: DysonLayoutProps) {
    const { metrics } = useSystemPulse();

    // Calculate breathing duration based on entropy (higher entropy = faster breathing)
    // Base: 4s, Min: 0.5s
    const breatheDuration = Math.max(0.5, 4 - (metrics.system_entropy * 3)) + 's';
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans selection:bg-dyson-plasma selection:text-white overflow-hidden relative">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none">
                <div
                    className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,51,51,0.03)_0%,rgba(5,5,5,0)_70%)] animate-breathe"
