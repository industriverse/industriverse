    ...
    async def optimize_energy(request: OptimizeRequest):
        ...
        try:
            ...
            if request.initial_state is not None:
                current_state = torch.tensor(request.initial_state, dtype=torch.float32)
            else:
                # Start from random noise
                current_state = torch.randn(32, 32)

            initial_energy = float(current_state.abs().sum().item())
            trajectory = [] if request.return_trajectory else None
            ...
            for iteration in range(request.max_iterations):
                ...
                energy_grad = energy_field.compute_energy_gradient(
                    current_state.unsqueeze(0).unsqueeze(0)
                )
                ...
                if request.method == "gradient":
                    # Gradient descent
                    learning_rate = 0.01
                    next_state = current_state - learning_rate * energy_grad.squeeze()

                elif request.method == "boltzmann":
                    ...
                    for _ in range(5):
                        ...
                        proposal = current_state - 0.01 * energy_grad.squeeze() + noise

                        # Compute energy
                        energy = float(proposal.abs().sum().item())
                        proposals.append(proposal)
                        energies.append(energy)

                    # Select via Boltzmann distribution
                    energies_tensor = torch.tensor(energies)
                    weights = torch.exp(-energies_tensor / request.temperature)
                    weights = weights / weights.sum()

                    selected_idx = torch.multinomial(weights, 1).item()
                    next_state = proposals[selected_idx]

                elif request.method == "annealing":
                    # Simulated annealing
                    temp = request.temperature * (1.0 - iteration / request.max_iterations)
                    noise_scale = temp * 0.1

                    next_state = current_state - 0.01 * energy_grad.squeeze()
                    next_state = next_state + torch.randn_like(next_state) * noise_scale

                else:
                    raise ValueError(f"Unknown method: {request.method}")

                # Compute new energy
                new_energy = float(next_state.abs().sum().item())

                # Check convergence
                energy_change = abs(new_energy - float(current_state.abs().sum().item()))
                if energy_change < request.tolerance:
                    converged = True
                ...
                if converged and request.target_energy is None:
                    break

                # Check target energy
                if request.target_energy is not None:
                    if abs(new_energy - request.target_energy) < request.tolerance:
                        converged = True
                        break

            # Final metrics
            final_energy = float(current_state.abs().sum().item())
            final_entropy = energy_field.compute_entropy(current_state)
            energy_reduction = initial_energy - final_energy

            processing_time = (time.time() - start_time) * 1000

            REQUEST_COUNT.labels(endpoint='/v1/optimize', status='success').inc()
            REQUEST_DURATION.labels(endpoint='/v1/optimize').observe(time.time() - start_time)

            return OptimizeResponse(
                optimized_state=current_state.cpu().numpy().tolist(),
            ...
            REQUEST_COUNT.labels(endpoint='/v1/optimize', status='error').inc()
