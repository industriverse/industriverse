...
class OptimizeRequest(BaseModel):
    """Request for energy optimization/equilibrium finding"""

    initial_state: Optional[List[List[float]]] = Field(
        None,
        description="Initial energy state (if None, starts from noise)"
    )
    ...
    method: str = Field(
        ...
        pattern="^(boltzmann|gradient|annealing)$"
    )

    temperature: float = Field(
        1.0,
        description="Temperature for Boltzmann sampling",
        gt=0.0
    )

    return_trajectory: bool = Field(
        False,
        description="Return optimization trajectory"
    )


class OptimizeResponse(BaseModel):
    """Response from energy optimization"""

    optimized_state: List[List[float]] = Field(
        ...,
        description="Optimized equilibrium state"
    )

    final_energy: float = Field(
        ...,
        description="Final energy level"
    )
    ...
    energy_reduction: float = Field(
        ...,
        description="Total energy reduction achieved"
    )
    ...
    trajectory: Optional[List[Dict[str, float]]] = Field(
        None,
        description="Optimization trajectory (if requested)"
    )
